...
def test_adaptive_sampler_outputs_psd_covariance_and_normalized_distribution():
    ...
    eigenvalues = torch.linalg.eigvalsh(outputs["distribution_process_covariance"])
    ...
    assert sampler.initial_state_head.weight.grad is not None
...
def test_samples_are_time_correlated_recursive_trajectories():
    ...
    with torch.no_grad():
        sampler.initial_state_head.weight.zero_()
        sampler.initial_state_head.bias.zero_()
