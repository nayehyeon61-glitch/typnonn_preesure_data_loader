...
from .probabilistic import conditional_distribution, hazard_logits_to_survival, joint_distribution


class SmallDualScaleModel(nn.Module):
    """One history encoder with distribution, survival and local-track heads."""
    ...
    def __init__(self, config: SmallModelConfig, bounds: EastAsiaBounds = EastAsiaBounds()):
        ...
        self.survival_head = nn.Sequential(
            nn.Linear(config.hidden_dim, config.hidden_dim), nn.GELU(),
            nn.Linear(config.hidden_dim, len(config.lead_days)),
        )
    ...
    def forward(self, history: torch.Tensor, history_mask: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        hazard_logits = self.survival_head(state)
        survival_probability = hazard_logits_to_survival(hazard_logits)
        q_t = conditional_distribution(distribution_logits)
        ...
        return {
            ...
            "p15": survival_probability[:, 0],
            "joint_distribution": joint_distribution(survival_probability, q_t),
            "track_latlon": torch.stack((lat, lon), dim=-1),
        }


class WeatherNextFusionTransformer(nn.Module):
    """Fuse masked forecast tokens with observed history and predict P15/Q_t/survival."""
    ...
    def __init__(
        ...
        self.history_projection = nn.Sequential(
            nn.Linear(model_config.input_dim * 2, hidden), nn.LayerNorm(hidden), nn.GELU()
        )
        ...
        layer = nn.TransformerEncoderLayer(
            d_model=transformer_config.model_dim, nhead=transformer_config.num_heads,
            dim_feedforward=transformer_config.feedforward_dim, dropout=transformer_config.dropout,
        ...
        if transformer_config.gpt_state_dim > 0:
            self.gpt_history_conditioner = nn.Sequential(
                nn.Linear(transformer_config.gpt_state_dim * 2, hidden * 2), nn.LayerNorm(hidden * 2),
                nn.GELU(), nn.Linear(hidden * 2, hidden * 2),
            )
        self.fusion = nn.Sequential(
            nn.Linear(hidden + transformer_config.model_dim, hidden), nn.LayerNorm(hidden), nn.GELU()
        )
        ...
        self.hazard_projection = nn.Linear(transformer_config.model_dim, 1)
    ...
    def _apply_input_mask(self, values, feature_mask, token_mask):
        ...
        if self.training and probability > 0:
            effective_feature_mask = effective_feature_mask & (torch.rand_like(values) >= probability)
    ...
    def forward(
        self, history, history_mask, forecast_values, forecast_feature_mask,
        forecast_token_mask, forecast_positions, gpt_state_values=None, gpt_state_mask=None,
        ...
        if self.gpt_history_conditioner is not None:
            ...
            masked_gpt_state = torch.where(gpt_state_mask.bool(), gpt_state_values, torch.zeros_like(gpt_state_values))
        ...
        forecast_memory = self.forecast_encoder(forecast_input, src_key_padding_mask=padding_mask)
        ...
        hazard_logits = self.hazard_projection(future_states).squeeze(-1)
        survival_probability = hazard_logits_to_survival(hazard_logits)
        q_t = conditional_distribution(distribution_logits)
